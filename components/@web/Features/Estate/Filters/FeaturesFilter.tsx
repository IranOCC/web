...
export const FeaturesFilter = ({ form, dataLoading, onSubmit }: any) => {
  return (
    <>
...
export const Features = ({ form, onSubmit }: { form: UseFormReturn<SearchEstateFormData, any, undefined>; onSubmit: any }) => {
  ...
  useEffect(() => {
    const get = async () => {
      setLoading(true);
      try {
        const data = await api.get("/tools/estate/feature/autoComplete", { params: { filter: { categories: searchParams?.get("filter[category]") || undefined } } });
