...
const AddEditOfficeCheckModal = ({ set }: any) => {
  ...
  const check = async () => {
    ...
    try {
      const d = await api.get(`tools/office/checking/${isNew ? "create" : "update"}`, { params: isNew ? undefined : { id: ID } });
      set(d.data);
      setOpenModal(false);
    } catch (error) {}
  };
...
export default AddEditOfficeCheckModal;
